...
export class YarnService {
  constructor(
    ...
    private readonly categoryModel: ModelType<ICategoryModel>,
  ...
  async getTag(_id: string): Promise<TagDto> {
    if (_id.length != 24)
      throw new NotFoundException('id not be 24 characters');
  ...
  async updateTag(dto: any): Promise<TagDto> {
    const { id } = dto;
    const updateTag = await this.tagModel
  ...
  async deleteTag(id: string): Promise<TagDto | ITagModel> {
    // что мы возвращаем ? промис от чего
  ...
  async categoryId(id: any): Promise<any> {
    const category = await this.categoryModel.findById(id);
    const candidate = await this.categoryAllAgregate({ title: category.title });
    if (!candidate) throw new NotFoundException('category not found');
    return candidate;
  }

  async categoryAllTitle() {
    return await this.categoryModel.find();
  }

  async categoryAllAgregate(item: any = {}) {
    let options = { ...item };
    // let options = {"title": "SandnesGarn"};
  ...
  async updateYarnItem(dto: any): Promise<any> {
    const { _id } = dto;
    const updateYarnItem = await this.yarnModel
      .findByIdAndUpdate(_id, dto, {
        new: true,
      })
      .exec();
